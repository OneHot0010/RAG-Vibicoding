...
|---------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [x] | 2026-05-06 | 目录结构、配置文件、main.py、虚拟环境已创建 |
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-05-06 | pytest 配置、测试目录约定、smoke import 测试、最小样例文档 |
| A3 | 配置加载与校验（Settings） | [x] | 2026-05-06 | Settings dataclass、YAML 加载、fail-fast 校验、main.py 启动检查、单元测试 |
...
| B9 | Azure Vision LLM 实现 | [x] | 2026-05-07 | AzureVisionLLM + image data URLs + deployment URL + factory registration + mock HTTP tests |
...
| C15 | 脚本入口 ingest.py | [x] | 2026-05-07 | Offline ingest CLI + path/collection/force + data-dir outputs + deterministic local embedding e2e tests |
...
| D6 | Reranker（Core 层编排 + Fallback） | [x] | 2026-05-08 | CoreReranker + libs.reranker adapter + fallback metadata + factory/trace tests |
| D7 | 脚本入口 query.py（查询可用） | [x] | 2026-05-08 | Query CLI + top-k/collection/verbose/no-rerank + offline embeddings + no-data/e2e tests |
...
- **完成内容**：
  - 新增 `JSONFormatter`，当 log message 为 dict 时原样作为 JSON 顶层输出。
  - 新增 `get_trace_logger(log_file="logs/traces.jsonl")`，自动创建目录并追加 JSON Lines。
  - 新增 `write_trace(trace_dict, log_file=...)`，将 trace payload 写入一行合法 JSON。
  - 避免同一路径重复注册 FileHandler，防止重复写行。
  - 支持 `TraceCollector(sink=lambda payload: write_trace(payload, ...))` 持久化 trace。
